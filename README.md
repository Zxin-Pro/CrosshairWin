...
设置窗口采用深色主题：左侧预设列表、右侧实时预览与参数分页，底部为保存/导入导出操作栏。
所有参数调整都会**立即反映到屏幕上的准星**，无需先点保存；点「取消」会还原到打开设置前的状态。

- **准星样式**：形状、颜色、透明度、大小、线宽、中心间隙、点/圆半径、描边
- **图片准星**：拖拽或选择导入，缩放方式、旋转、透明度、偏移、原色/着色
- **位置与显示**：目标显示器、位置微调、启动行为
- **快捷键**：点击输入框后直接按组合键即可修改
